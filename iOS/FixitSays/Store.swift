...
/// StoreKit 2 — auto-renewable monthly subscription (`fixitsays_pro_monthly`).
...
final class Store: ObservableObject {
    static let productID = "fixitsays_pro_monthly"
    ...
    private var debugForcePro: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["FIXITSAYS_FORCE_PRO"] == "1"
        #else
        return false
        #endif
    }
    ...
    init() {
        ...
        Task {
            #if DEBUG
            // FIXITSAYS_NO_SK skips the product fetch so headless screenshot runs (no StoreKit config /
            // no App Store account) don't trigger the system "Sign in to Apple Account" prompt.
            if ProcessInfo.processInfo.environment["FIXITSAYS_NO_SK"] != "1" { await loadProduct() }
            #else
            await loadProduct()
            #endif
