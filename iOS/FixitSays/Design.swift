...
extension Color {
    static let fixitsaysAccent = Color(hex: "#007AFF")            // the single accent
    static let fixitsaysCard = Color(uiColor: .secondarySystemBackground)
    static let fixitsaysCard2 = Color(uiColor: .tertiarySystemBackground)
    static let fixitsaysField = Color(uiColor: .tertiarySystemFill)
    static let fixitsaysHair = Color(uiColor: .separator)
}

// MARK: - Flat surfaces (cards / pills / buttons)

extension View {
    func fixitsaysCard(cornerRadius: CGFloat = 20) -> some View {
        self.padding(16)
            .background(Color.fixitsaysCard, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
    }

    func fixitsaysPill() -> some View {
        self.padding(.horizontal, 14).padding(.vertical, 8)
            .background(Color.fixitsaysCard, in: Capsule())
    }
...
struct FilledAccentButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(Color.fixitsaysAccent, in: Capsule())
...
struct FixitSaysBackground: View {
